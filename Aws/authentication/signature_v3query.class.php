...
class AuthV3Query extends Signer implements Signable
{
    ...
    public function authenticate()
    {
        ...
        $current_time   = time();
        $date           = gmdate(
            CFUtilities::DATE_FORMAT_RFC2616, $current_time
        );
        $timestamp      = gmdate(
            CFUtilities::DATE_FORMAT_ISO8601, $current_time
        );
        ...
        $return_curl_handle = isset($query['returnCurlHandle'])
            ? $query['returnCurlHandle'] : false;
        ...
        $domain = str_replace(
            array('http://', 'https://'), '', $this->endpoint
        );
        ...
        if (isset($parsed_url['port']) && (integer)$parsed_url['port'] !== 80
            && (integer)$parsed_url['port'] !== 443
        ) {
            $host_header
                = strtolower($parsed_url['host']) . ':' . $parsed_url['port'];
        } else {
        ...
        $signature = base64_encode(
            hash_hmac('sha256', $this->string_to_sign, $this->secret_key, true)
        );
        ...
        $request->add_header(
            'X-Amzn-Authorization', $headers['X-Amzn-Authorization']
        );
