...
function __aws_sdk_ua_callback()
{
    ...
    if ($extensions) {
        foreach ($extensions as $extension) {
            if ($extension === 'curl' && function_exists('curl_version')) {
                ...
                $sorted_extensions[strtolower($extension)]
                              = $curl_version['version'];
            } elseif ($extension === 'pcre' && defined('PCRE_VERSION')) {
                $pcre_version                              = explode(
                    ' ', PCRE_VERSION
                );
                ...
                $openssl_version = explode(
                    ' ', OPENSSL_VERSION_TEXT
                );
                $sorted_extensions[strtolower($extension)]
                                 = $openssl_version[1];
            } else {
                $sorted_extensions[strtolower($extension)] = phpversion(
                    $extension
                );
            }
        }
    ...
    foreach (
        array('simplexml', 'json', 'pcre', 'spl', 'curl', 'openssl', 'apc',
              'xcache', 'memcache', 'memcached', 'pdo', 'pdo_sqlite', 'sqlite',
              'sqlite3', 'zlib', 'xdebug') as $ua_ext
    ) {
    ...
    foreach (
        array('memory_limit', 'date.timezone', 'open_basedir', 'safe_mode',
              'zend.enable_gc') as $cfg
    ) {
        ...
        if (in_array($cfg, array('memory_limit', 'date.timezone'), true)) {
            ...
            $cfg, array('open_basedir', 'safe_mode', 'zend.enable_gc'), true
        )) {
            if ($cfg_value === false || $cfg_value === '' || $cfg_value === 0) {
                $cfg_value = 'off';
            } elseif ($cfg_value === true || $cfg_value === '1'
                || $cfg_value === 1
            ) {
                $cfg_value = 'on';
            }
...
define('CFRUNTIME_USERAGENT',
    CFRUNTIME_NAME . '/' . CFRUNTIME_VERSION . ' PHP/' . PHP_VERSION . ' '
    . str_replace(' ', '_', php_uname('s')) . '/' . str_replace(
        ' ', '_', php_uname('r')
    ) . ' Arch/' . php_uname('m') . ' SAPI/' . php_sapi_name() . ' Integer/'
    . PHP_INT_MAX . ' Build/' . CFRUNTIME_BUILD . __aws_sdk_ua_callback());
...
class CFRuntime
{
    ...
    public function __construct(array $options = array())
    {
        ...
        $this->credentials = CFCredentials::get(
            (isset($options['credentials']) ? $options['credentials']
                : CFCredentials::DEFAULT_KEY)
        );
        $this->credentials = new CFCredential(
            array_merge($this->credentials->to_array(), $options)
        );
        ...
        if (isset($options['key']) && isset($options['secret'])) {
            ...
            $this->auth_token = isset($options['token']) ? $options['token']
                : null;
    ...
    public static function factory(array $options = array())
    {
        if (version_compare(PHP_VERSION, '5.3.0', '<')) {
            throw new Exception(
                'PHP 5.3 or newer is required to instantiate a new class with CLASS::factory().'
            );
        }
    ...
    public function  __call($name, $arguments)
    {
        ...
        $method_name = strtolower(
            preg_replace('/([a-z])([A-Z])/', '$1_$2', $name)
        );
        ...
        throw new CFRuntime_Exception(
            'The method ' . $name . '() is undefined. Attempted to map to '
            . $method_name . '() which is also undefined. Error occurred'
        );
    }
    ...
    {
        trigger_error(
            'Disabling SSL connections is potentially unsafe and highly discouraged.',
            E_USER_WARNING
        );
        $this->use_ssl = false;
    ...
    {
        trigger_error(
            'Disabling the verification of SSL certificates can lead to man-in-the-middle attacks. It is potentially unsafe and highly discouraged.',
            E_USER_WARNING
        );
    ...
    {
        if (substr($operation, 0, strlen($this->operation_prefix))
            !== $this->operation_prefix
        ) {
        ...
        $return_curl_handle = isset($payload['returnCurlHandle'])
            ? $payload['returnCurlHandle'] : false;
        ...
        if ($this->use_cache_flow) {
            ...
            $cache_id
                = $this->key . '_' . get_class($this) . '_' . $operation . '_'
        ...
        // Signer
        $signer                  = new $this->auth_class(
            $this->hostname, $operation, $payload, $this->credentials
        );
        $signer->key             = $this->key;
        ...
        $signer->registered_streaming_read_callback
                                 = $this->registered_streaming_read_callback;
        $signer->registered_streaming_write_callback
        ...
        $data = new $this->response_class(
            $headers,
            $this->parse_callback($request->get_response_body(), $headers),
        ...
        if (
            ((integer)$request->get_response_code() === 400
                && (string)$request->get_response_body()
                === '{"__type": "com.amazon.coral.availability#ThrottlingException"}')
            || // Bad Request (throttled)
            (integer)$request->get_response_code() === 500
            || // Internal Error (presumably transient)
            (integer)$request->get_response_code() === 503
        )   // Service Unavailable (presumably transient)
        {
    ...
    public function send($clear_after_send = true)
    {
        if ($this->use_batch_flow) {
            ...
            if (!$this->use_cache_flow) {
                ...
                $parsed_data = array_map(
                    array($this, 'parse_callback'), $response
                );
            ...
            // Generate an identifier specific to this particular set of arguments.
            $cache_id = $this->key . '_' . get_class($this) . '_' . sha1(
                    serialize($this->batch_object)
                );
            ...
            // Invoke the cache callback function to determine whether to pull data from the cache or make a fresh request.
            $data_set    = $this->cache_object->response_manager(
                array($this, 'cache_callback_batch'), array($this->batch_object)
            );
        ...
        throw new CFBatchRequest_Exception(
    ...
    public function parse_callback($response, $headers = null)
    {
        ...
        if (isset($headers['content-encoding'])) {
            switch (strtolower(
                trim($headers['content-encoding'], "\x09\x0A\x0D\x20")
            )) {
                case 'gzip':
                case 'x-gzip':
                    if (strpos($headers['_info']['url'], 'monitoring.')
                        !== false
                    ) {
    ...
    public function cache($expires)
    {
        ...
        if (!$this->cache_class) {
            throw new CFRuntime_Exception(
                'Must call set_cache_config() before using cache()'
            );
        }
    ...
    public function cache_callback(
        $action, $opt = null, $domain = null, $signature_version = 2
    ) {
        ...
        $response = $this->authenticate(
            $action, $opt, $domain, $signature_version
        );
...
class CFLoader
{
    ...
    public static function autoloader($class)
    {
        ...
        if (strstr($class, 'Amazon')) {
            require_once
                $path . 'services' . DIRECTORY_SEPARATOR . str_ireplace(
                    'Amazon', '', strtolower($class)
                ) . '.class.php';
        } // Utility classes
        elseif (strstr($class, 'CF')) {
            require_once
                $path . 'utilities' . DIRECTORY_SEPARATOR . str_ireplace(
                    'CF', '', strtolower($class)
                ) . '.class.php';
        } // Load CacheCore
        elseif (strstr($class, 'Cache')) {
            require_once $path . 'lib' . DIRECTORY_SEPARATOR . 'cachecore'
                . DIRECTORY_SEPARATOR . strtolower($class) . '.class.php';
        } // Load RequestCore
        elseif (strstr($class, 'RequestCore')
            || strstr(
                $class, 'ResponseCore'
            )
        ) {
            require_once $path . 'lib' . DIRECTORY_SEPARATOR . 'requestcore'
                . DIRECTORY_SEPARATOR . 'requestcore.class.php';
        } // Load Authentication Signers
        elseif (strstr($class, 'Auth')) {
            require_once
                $path . 'authentication' . DIRECTORY_SEPARATOR . str_replace(
                    'auth', 'signature_', strtolower($class)
                ) . '.class.php';
        } // Load Signer interface
        elseif ($class === 'Signer') {
            if (!interface_exists('Signable', false)) {
                require_once $path . 'authentication' . DIRECTORY_SEPARATOR
                    . 'signable.interface.php';
            }
        ...
        elseif (strstr($class, 'sfYaml')) {
            require_once $path . 'lib' . DIRECTORY_SEPARATOR . 'yaml'
...
if (file_exists(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'config.inc.php')) {
    ...
    if (!isset($_ENV['HOME']) && isset($_SERVER['HOME'])) {
        ...
        if (!$_ENV['HOME']) {
            switch (strtolower(PHP_OS)) {
                ...
                case 'win32':
                    $_ENV['HOME']
                        = 'c:\Documents and Settings' . DIRECTORY_SEPARATOR
                        . get_current_user();
                    break;
    ...
    if (getenv('HOME')
        && file_exists(
            getenv('HOME') . DIRECTORY_SEPARATOR . '.aws' . DIRECTORY_SEPARATOR
            . 'sdk' . DIRECTORY_SEPARATOR . 'config.inc.php'
        )
    ) {
        include_once
            getenv('HOME') . DIRECTORY_SEPARATOR . '.aws' . DIRECTORY_SEPARATOR
