...
class AmazonEMR extends CFRuntime
{
     ...
     */
    public function add_instance_groups(
        $instance_groups, $job_flow_id, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            ...
            array(
                'InstanceGroups' => (is_array($instance_groups)
    ...
    public function describe_job_flows($opt = null)
    {
        ...
        if (isset($opt['CreatedAfter'])) {
            $opt['CreatedAfter'] = $this->util->convert_date_to_iso8601(
                $opt['CreatedAfter']
            );
        }
        ...
        if (isset($opt['CreatedBefore'])) {
            $opt['CreatedBefore'] = $this->util->convert_date_to_iso8601(
                $opt['CreatedBefore']
            );
        }
        ...
        if (isset($opt['JobFlowIds'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'JobFlowIds' => (is_array($opt['JobFlowIds'])
                        ? $opt['JobFlowIds'] : array($opt['JobFlowIds']))
                ), 'member'
            )
            );
        ...
        if (isset($opt['JobFlowStates'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'JobFlowStates' => (is_array($opt['JobFlowStates'])
                        ? $opt['JobFlowStates'] : array($opt['JobFlowStates']))
                ), 'member'
            )
            );
    ...
    public function run_job_flow($name, $instances, $opt = null)
    {
        ...
        if (isset($opt['SupportedProducts'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'SupportedProducts' => (is_array($opt['SupportedProducts'])
                        ? $opt['SupportedProducts']
                        : array($opt['SupportedProducts']))
                ), 'member'
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function set_termination_protection(
        $job_flow_ids, $termination_protected, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'JobFlowIds' => (is_array($job_flow_ids) ? $job_flow_ids
                    : array($job_flow_ids))
            ), 'member'
        )
        );
