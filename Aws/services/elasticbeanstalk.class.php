...
class AmazonElasticBeanstalk extends CFRuntime
{
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_application_version(
        $application_name, $version_label, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_configuration_template(
        $application_name, $template_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function create_environment(
        $application_name, $environment_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_application_version(
        $application_name, $version_label, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_configuration_template(
        $application_name, $template_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_environment_configuration(
        $application_name, $environment_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
    ...
    public function describe_application_versions($opt = null)
    {
        ...
        if (isset($opt['VersionLabels'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'VersionLabels' => (is_array($opt['VersionLabels'])
                        ? $opt['VersionLabels'] : array($opt['VersionLabels']))
                ), 'member'
            )
            );
    ...
    public function describe_applications($opt = null)
    {
        ...
        if (isset($opt['ApplicationNames'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'ApplicationNames' => (is_array($opt['ApplicationNames'])
                        ? $opt['ApplicationNames']
                        : array($opt['ApplicationNames']))
                ), 'member'
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function describe_configuration_settings(
        $application_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function describe_environments($opt = null)
    {
        ...
        if (isset($opt['IncludedDeletedBackTo'])) {
            $opt['IncludedDeletedBackTo']
                = $this->util->convert_date_to_iso8601(
                $opt['IncludedDeletedBackTo']
            );
        }
        ...
        if (isset($opt['EnvironmentIds'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'EnvironmentIds' => (is_array($opt['EnvironmentIds'])
                        ? $opt['EnvironmentIds']
                        : array($opt['EnvironmentIds']))
                ), 'member'
            )
            );
        ...
        if (isset($opt['EnvironmentNames'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'EnvironmentNames' => (is_array($opt['EnvironmentNames'])
                        ? $opt['EnvironmentNames']
                        : array($opt['EnvironmentNames']))
                ), 'member'
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function update_application_version(
        $application_name, $version_label, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function update_configuration_template(
        $application_name, $template_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function validate_configuration_settings(
        $application_name, $option_settings, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'OptionSettings' => (is_array($option_settings)
                    ? $option_settings : array($option_settings))
            ), 'member'
        )
        );
