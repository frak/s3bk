...
class AmazonSDB extends CFRuntime
{
     ...
     */
    public static function remap_batch_items_for_complextype(
        $items, $replace = false
    ) {
        $map = array(
            'Item' => array()
                ...
                foreach ($value as $k => $v) {
                    ...
                    foreach ($v as $vv) {
                        ...
                        if (
                            ...
                            || (isset($replace[$key])
                                && array_search($k, $replace[$key], true)
                                !== false)
                        ) {
                            $n['Replace'] = 'true';
                        }
     ...
     * @return array A <CFComplexType>-compatible mapping of parameters.
     */
    public static function remap_attribute_items_for_complextype(
        $keys, $replace = false
    ) {
        $map = array(
            'Attribute' => array()
        ...
        foreach ($keys as $k => $v) {
            ...
            foreach ($v as $vv) {
                ...
                if (
                    $replace === (boolean)true
                    || (is_array($replace)
                        && array_search($k, $replace, true) !== false)
                ) {
                    $n['Replace'] = 'true';
                }
     ...
     */
    public function batch_delete_attributes(
        $domain_name, $item_keypairs, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        return $this->authenticate(
            'BatchDeleteAttributes', $opt, $this->hostname
        );
    }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function batch_put_attributes(
        $domain_name, $item_keypairs, $replace = null, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function delete_attributes(
        $domain_name, $item_name, $attributes = null, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        if ($attributes) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'Attribute' => (is_array($attributes) ? $attributes
                        : array($attributes))
                )
            )
            );
    ...
    public function get_attributes(
        $domain_name, $item_name, $attribute_name = null, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        if ($attribute_name) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'AttributeName' => (is_array($attribute_name)
    ...
    public function put_attributes(
        $domain_name, $item_name, $keypairs, $replace = null, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
