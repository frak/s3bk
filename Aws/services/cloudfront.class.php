...
class AmazonCloudFront extends CFRuntime
{
    ...
    public function __construct(array $options = array())
    {
        ...
        $this->base_xml
            =
            '<?xml version="1.0" encoding="UTF-8"?><%s xmlns="http://cloudfront.amazonaws.com/doc/'
            . $this->api_version . '/"></%1$s>';

        // Set a default key pair ID and private key
        if (isset($options['credentials'])) {
            $this->key_pair_id = CFCredentials::get(
                $options['credentials']
            )->cloudfront_keypair;
            $this->private_key = CFCredentials::get(
                $options['credentials']
            )->cloudfront_pem;
        } else {
    ...
    public function authenticate(
        $method = 'GET', $path = null, $opt = null, $xml = null, $etag = null,
        $redirects = 0
    ) {
        if (!$opt) {
            $opt = array();
        ...
        if ($this->use_cache_flow) {
            ...
            $cache_id
                = $this->key . '_' . get_class($this) . '_' . $method . sha1(
                    $path
                ) . '_' . sha1(serialize($method_arguments));
        ...
        if (isset($opt['query_string']) && count($opt['query_string'])) {
            $querystring
                = '?' . $this->util->to_query_string($opt['query_string']);
        }
        ...
        $request = new $this->request_class(
            $request_url, $this->proxy, $helpers, $this->credentials
        );
        ...
        $canonical_date = gmdate(
            $this->util->konst($this->util, 'DATE_FORMAT_RFC2616')
        );
        $request->add_header('x-amz-date', $canonical_date);
        $signature = base64_encode(
            hash_hmac('sha1', $canonical_date, $this->secret_key, true)
        );
        $request->add_header(
        ...
        if ($this->use_batch_flow) {
            ...
            && $opt['returnCurlHandle'] == (bool)true
        ) {
        ...
        if ((integer)$request->get_response_code() === 500
            ...
            if ($redirects <= $this->max_retries) {
                ...
                $data = $this->authenticate(
                    $method, $path, $opt, $xml, $etag, ++$redirects
                );
            }
        }
    ...
    {
        throw new CloudFront_Exception(
            'SSL/HTTPS is REQUIRED for Amazon CloudFront and cannot be disabled.'
        );
    }
    ...
    public function generate_config_xml($origin, $caller_reference, $opt = null)
    {
        ...
        $xml = simplexml_load_string(
            sprintf(
                $this->base_xml, (
            (isset($opt['Streaming']) && $opt['Streaming'] == (bool)true)
                ? 'StreamingDistributionConfig' : 'DistributionConfig')
            )
        );

        if (substr($origin, 0, 7) === 'http://'
            || substr($origin, 0, 8) === 'https://'
        ) {
            // Custom Origin
            $custom_origin = $xml->addChild('CustomOrigin');
            $custom_origin->addChild(
                'DNSName',
                str_replace(array('http://', 'https://'), '', $origin)
            );

            if (isset($opt['OriginProtocolPolicy'])) {
                $custom_origin->addChild(
                    'OriginProtocolPolicy', $opt['OriginProtocolPolicy']
                );
            } else {
                $custom_origin->addChild(
            ...
            $s3_origin->addChild(
                'DNSName',
                $origin . ((stripos($origin, '.s3.amazonaws.com') === false)
                    ? '.s3.amazonaws.com' : '')
            );
        ...
        if (isset($opt['Logging'])) {
            if (is_array($opt['Logging'])) {
                ...
                $logging->addChild(
                    'Bucket', $bucket_name . (
                    (stripos($bucket_name, '.s3.amazonaws.com') === false)
                        ? '.s3.amazonaws.com' : ''
                    )
                );
        ...
        if (isset($opt['RequiredProtocols'])) {
            ...
            $required_protocols->addChild(
                'Protocol', $opt['RequiredProtocols']
            );
        }
    ...
    public function update_config_xml($xml, $opt = null)
    {
        ...
        if (isset($xml->S3Origin)) {
            ...
            if (isset($opt['OriginAccessIdentity'])) {
                ...
                $origin->addChild(
                    'OriginAccessIdentity', $xml->OriginAccessIdentity
                );
            }
        ...
        if (isset($opt['Logging'])) {
            if (is_array($opt['Logging'])) {
                ...
                $logging->addChild(
                    'Bucket',
                    $bucket_name . ((stripos($bucket_name, '.s3.amazonaws.com')
                        === false) ? '.s3.amazonaws.com' : '')
                );
        ...
        } elseif (isset($xml->TrustedSigners)
            && $xml->TrustedSigners->count()
        ) {
            $trusted_signers = $update->addChild('TrustedSigners');

            // Handle 'Self' vs. everything else
            foreach (
                $xml->TrustedSigners->children() as $signer_key => $signer_value
            ) {
                if (strtolower((string)$signer_key) === 'self') {
                    $trusted_signers->addChild('Self');
                } else {
                    $trusted_signers->addChild(
                        'AwsAccountNumber', (string)$signer_value
                    );
                }
            }
        }
    ...
    public function remove_cname($xml, $cname)
    {
        ...
        if (isset($xml->CNAME)) {
            ...
            if (is_array($cname)) {
                foreach ($cname as $cn) {
                    for (
                        $i = 0, $length = sizeof($xml->CNAME); $i < $length;
                        $i++
                    ) {
    ...
    public function generate_oai_xml($caller_reference, $opt = null)
    {
        // Default, empty XML
        $xml = simplexml_load_string(
            sprintf($this->base_xml, 'CloudFrontOriginAccessIdentityConfig')
        );
    ...
    public function generate_invalidation_xml($caller_reference, $opt = null)
    {
        // Default, empty XML
        $xml = simplexml_load_string(
            sprintf($this->base_xml, 'InvalidationBatch')
        );
        ...
        // Paths
        if (isset($opt['Paths'])) {
            $paths = is_array($opt['Paths']) ? $opt['Paths']
                : array($opt['Paths']);
    ...
    public function create_distribution($origin, $caller_reference, $opt = null)
    {
        ...
        $path
             =
            '/' . ((isset($opt['Streaming']) && $opt['Streaming'] == (bool)true)
                ? 'streaming-distribution' : 'distribution');
     ...
     */
    public function set_distribution_config(
        $distribution_id, $xml, $etag, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function get_oai_config($identity_id, $opt = null)
    {
        ...
        $path
            = '/origin-access-identity/cloudfront/' . $identity_id . '/config';
     ...
     */
    public function create_invalidation(
        $distribution_id, $caller_reference, $paths, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     */
    public function get_invalidation(
        $distribution_id, $invalidation_id, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     */
    public function get_private_object_url(
        $distribution_hostname, $filename, $expires, $opt = null
    ) {
        if (!$this->key_pair_id || !$this->private_key) {
            throw new CloudFront_Exception(
                'You must set both a Amazon CloudFront keypair ID and an RSA private key for that keypair before using '
                . __FUNCTION__ . '()'
            );
        }
        if (!function_exists('openssl_sign')) {
            throw new CloudFront_Exception(
                __FUNCTION__
                . '() uses functions from the OpenSSL PHP Extension <http://php.net/openssl>, which is not installed in this PHP installation'
            );
        }
        ...
        switch (substr($distribution_hostname, 0, 1) === 's') {
            ...
            default:
                $scheme = 'http';
                $scheme .= (isset($opt['Secure']) && $opt['Secure'] === true
                    ? 's' : '');
                $resource = $scheme . '://' . $distribution_hostname . '/'
                    . str_replace(
                        array('%3F', '%3D', '%26', '%2F'),
                        array('?', '=', '&', '/'), rawurlencode($filename)
                    );
        ...
        . str_replace(
            array('%3F', '%3D', '%26', '%2F'), array('?', '=', '&', '/'),
            rawurlencode($filename)
        )
        . $conjunction
        . ($expiration_key === 'Expires' ? ($expiration_key . '=' . $expires)
            : ($expiration_key . '=' . $encoded_policy))
