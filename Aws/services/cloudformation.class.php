...
class AmazonCloudFormation extends CFRuntime
{
    ...
    public function create_stack($stack_name, $opt = null)
    {
        ...
        if (isset($opt['NotificationARNs'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'NotificationARNs' => (is_array($opt['NotificationARNs'])
                        ? $opt['NotificationARNs']
                        : array($opt['NotificationARNs']))
                ), 'member'
            )
            );
        ...
        if (isset($opt['Capabilities'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'Capabilities' => (is_array($opt['Capabilities'])
                        ? $opt['Capabilities'] : array($opt['Capabilities']))
                ), 'member'
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function describe_stack_resource(
        $stack_name, $logical_resource_id, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function list_stacks($opt = null)
    {
        ...
        if (isset($opt['StackStatusFilter'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'StackStatusFilter' => (is_array($opt['StackStatusFilter'])
                        ? $opt['StackStatusFilter']
                        : array($opt['StackStatusFilter']))
                ), 'member'
            )
            );
