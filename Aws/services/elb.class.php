...
class AmazonELB extends CFRuntime
{
     ...
     */
    public function apply_security_groups_to_load_balancer(
        $load_balancer_name, $security_groups, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            ...
            array(
                'SecurityGroups' => (is_array($security_groups)
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function attach_load_balancer_to_subnets(
        $load_balancer_name, $subnets, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function configure_health_check(
        $load_balancer_name, $health_check, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'HealthCheck' => (is_array($health_check) ? $health_check
                    : array($health_check))
            ), 'member'
        )
        );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_app_cookie_stickiness_policy(
        $load_balancer_name, $policy_name, $cookie_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_lb_cookie_stickiness_policy(
        $load_balancer_name, $policy_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_load_balancer(
        $load_balancer_name, $listeners, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        if (isset($opt['AvailabilityZones'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'AvailabilityZones' => (is_array($opt['AvailabilityZones'])
                        ? $opt['AvailabilityZones']
                        : array($opt['AvailabilityZones']))
                ), 'member'
            )
            );
        ...
        if (isset($opt['Subnets'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'Subnets' => (is_array($opt['Subnets']) ? $opt['Subnets']
                        : array($opt['Subnets']))
                ), 'member'
            )
            );
        ...
        if (isset($opt['SecurityGroups'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'SecurityGroups' => (is_array($opt['SecurityGroups'])
                        ? $opt['SecurityGroups']
                        : array($opt['SecurityGroups']))
                ), 'member'
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_load_balancer_listeners(
        $load_balancer_name, $listeners, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_load_balancer_policy(
        $load_balancer_name, $policy_name, $policy_type_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_load_balancer_listeners(
        $load_balancer_name, $load_balancer_ports, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'LoadBalancerPorts' => (is_array($load_balancer_ports)
                    ? $load_balancer_ports : array($load_balancer_ports))
            ), 'member'
        )
        );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_load_balancer_policy(
        $load_balancer_name, $policy_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function deregister_instances_from_load_balancer(
        $load_balancer_name, $instances, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function describe_load_balancer_policies($opt = null)
    {
        ...
        if (isset($opt['PolicyNames'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'PolicyNames' => (is_array($opt['PolicyNames'])
                        ? $opt['PolicyNames'] : array($opt['PolicyNames']))
                ), 'member'
            )
            );
    ...
    public function describe_load_balancer_policy_types($opt = null)
    {
        ...
        if (isset($opt['PolicyTypeNames'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'PolicyTypeNames' => (is_array($opt['PolicyTypeNames'])
                        ? $opt['PolicyTypeNames']
                        : array($opt['PolicyTypeNames']))
                ), 'member'
            )
            );
    ...
    public function describe_load_balancers($opt = null)
    {
        ...
        if (isset($opt['LoadBalancerNames'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'LoadBalancerNames' => (is_array($opt['LoadBalancerNames'])
                        ? $opt['LoadBalancerNames']
                        : array($opt['LoadBalancerNames']))
                ), 'member'
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function detach_load_balancer_from_subnets(
        $load_balancer_name, $subnets, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function disable_availability_zones_for_load_balancer(
        $load_balancer_name, $availability_zones, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'AvailabilityZones' => (is_array($availability_zones)
                    ? $availability_zones : array($availability_zones))
            ), 'member'
        )
        );

        return $this->authenticate(
            'DisableAvailabilityZonesForLoadBalancer', $opt
        );
    }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function enable_availability_zones_for_load_balancer(
        $load_balancer_name, $availability_zones, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        ...
        return $this->authenticate(
            'EnableAvailabilityZonesForLoadBalancer', $opt
        );
    }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function register_instances_with_load_balancer(
        $load_balancer_name, $instances, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function set_load_balancer_listener_ssl_certificate(
        $load_balancer_name, $load_balancer_port, $ssl_certificate_id,
        $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        ...
        return $this->authenticate(
            'SetLoadBalancerListenerSSLCertificate', $opt
        );
    }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function set_load_balancer_policies_for_backend_server(
        $load_balancer_name, $instance_port, $policy_names, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'PolicyNames' => (is_array($policy_names) ? $policy_names
                    : array($policy_names))
            ), 'member'
        )
        );
        ...
        return $this->authenticate(
            'SetLoadBalancerPoliciesForBackendServer', $opt
        );
    }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function set_load_balancer_policies_of_listener(
        $load_balancer_name, $load_balancer_port, $policy_names, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
