...
class AmazonRDS extends CFRuntime
{
     ...
     */
    public function authorize_db_security_group_ingress(
        $db_security_group_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_db_instance(
        $db_instance_identifier, $allocated_storage, $db_instance_class,
        $engine, $master_username, $master_user_password, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        if (isset($opt['DBSecurityGroups'])) {
            $opt = array_merge(
                ...
                array(
                    'DBSecurityGroups' => (is_array($opt['DBSecurityGroups'])
                        ? $opt['DBSecurityGroups']
                        : array($opt['DBSecurityGroups']))
                ), 'member'
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_db_instance_read_replica(
        $db_instance_identifier, $source_db_instance_identifier, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_db_parameter_group(
        $db_parameter_group_name, $db_parameter_group_family, $description,
        $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function create_db_security_group(
        $db_security_group_name, $db_security_group_description, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function create_db_snapshot(
        $db_snapshot_identifier, $db_instance_identifier, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_db_parameter_group(
        $db_parameter_group_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_db_security_group(
        $db_security_group_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function describe_db_parameters(
        $db_parameter_group_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function describe_engine_default_parameters(
        $db_parameter_group_family, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function describe_reserved_db_instances_offerings($opt = null)
    {
        ...
        return $this->authenticate(
            'DescribeReservedDBInstancesOfferings', $opt
        );
    }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function modify_db_parameter_group(
        $db_parameter_group_name, $parameters, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function purchase_reserved_db_instances_offering(
        $reserved_db_instances_offering_id, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        $opt['ReservedDBInstancesOfferingId']
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function reset_db_parameter_group(
        $db_parameter_group_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function restore_db_instance_from_db_snapshot(
        $db_instance_identifier, $db_snapshot_identifier, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function restore_db_instance_to_point_in_time(
        $source_db_instance_identifier, $target_db_instance_identifier,
        $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        ...
        if (isset($opt['RestoreTime'])) {
            $opt['RestoreTime'] = $this->util->convert_date_to_iso8601(
                $opt['RestoreTime']
            );
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function revoke_db_security_group_ingress(
        $db_security_group_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
