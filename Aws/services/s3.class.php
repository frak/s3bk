...
class AmazonS3 extends CFRuntime
{
    ...
    public function __construct(array $options = array())
    {
        ...
        $this->base_acp_xml
            = '<?xml version="1.0" encoding="UTF-8"?><AccessControlPolicy xmlns="http://s3.amazonaws.com/doc/latest/"></AccessControlPolicy>';
        $this->base_location_constraint
            =
            '<?xml version="1.0" encoding="UTF-8"?><CreateBucketConfiguration xmlns="http://s3.amazonaws.com/doc/'
            . $this->api_version
            . '/"><LocationConstraint></LocationConstraint></CreateBucketConfiguration>';
        $this->base_logging_xml
            =
            '<?xml version="1.0" encoding="utf-8"?><BucketLoggingStatus xmlns="http://doc.s3.amazonaws.com/'
            . $this->api_version . '"></BucketLoggingStatus>';
        ...
        $this->base_versioning_xml
            =
            '<?xml version="1.0" encoding="utf-8"?><VersioningConfiguration xmlns="http://s3.amazonaws.com/doc/'
            . $this->api_version . '/"></VersioningConfiguration>';
        ...
        $this->website_config_xml
            =
            '<?xml version="1.0" encoding="utf-8"?><WebsiteConfiguration xmlns="http://s3.amazonaws.com/doc/'
            . $this->api_version
            . '/"><IndexDocument><Suffix>index.html</Suffix></IndexDocument><ErrorDocument><Key>error.html</Key></ErrorDocument></WebsiteConfiguration>';
    ...
    public function authenticate(
        $bucket, $opt = null, $location = null, $redirects = 0, $nothing = null
    ) {
        /*
        ...
        if (!$this->validate_bucketname_support($bucket)) {
            ...
            throw new S3_Exception(
                'S3 does not support "' . $bucket
                . '" as a valid bucket name. Review "Bucket Restrictions and Limitations" in the S3 Developer Guide for more information.'
            );
        ...
        if ($this->use_cache_flow) {
            ...
            $cache_id
                = $this->key . '_' . get_class($this) . '_' . $bucket . '_'
                . sha1(serialize($method_arguments));

            // Instantiate the appropriate caching object.
            $this->cache_object = new $this->cache_class(
                $cache_id, $this->cache_location, $this->cache_expires,
                $this->cache_compress
            );
            ...
            $data = $this->cache_object->response_manager(
                array($this, 'cache_callback'), $method_arguments
            );
        ...
        {
            $hostname
                = $this->hostname . $this->resource_prefix . (($bucket === ''
                    || $this->resource_prefix === '/' . $bucket) ? ''
                    : ('/' . $bucket));
        } else {
            $hostname = $this->vhost
                ? $this->vhost
        ...
        $date = gmdate(
            CFUtilities::DATE_FORMAT_RFC2616,
            (time() + (integer)$this->adjust_offset)
        );
        ...
        if (isset($opt['query_string'])) {
            $querystringparams = array_merge(
                $querystringparams, $opt['query_string']
            );
        }
        ...
        $signable_query_string = $this->util->to_query_string(
            $signable_querystringparams
        );
        ...
        if (isset($opt['sub_resource'])) {
            $signable_resource .= $conjunction . rawurlencode(
                    $opt['sub_resource']
                );
            $conjunction = '&';
        }
        ...
        $this->request_url
            = $scheme . $hostname . $signable_resource . $signable_query_string
            . $non_signable_resource;
        ...
        $request = new $this->request_class(
            $this->request_url, $this->proxy, $helpers, $this->credentials
        );
        ...
        if ($this->registered_streaming_read_callback) {
            $request->register_streaming_read_callback(
                $this->registered_streaming_read_callback
            );
        }

        if ($this->registered_streaming_write_callback) {
            $request->register_streaming_write_callback(
        ...
        if (isset($opt['fileUpload'])) {
            if (is_resource($opt['fileUpload'])) {
                ...
                $length
                    = null; // From current position until EOF by default, size determined by set_read_stream()
                ...
                $length
                    = $request->read_stream_size; // The file size by default
                ...
                if ($headers['Content-Type']
                    ...
                    $mime_type               = CFMimeTypes::get_mimetype(
                        $extension
                    );
        ...
        if (isset($opt['body'])) {
            ...
            if (!isset($opt['NoContentMD5']) || $opt['NoContentMD5'] !== true) {
                $headers['Content-MD5'] = $this->util->hex_to_base64(
                    md5($opt['body'])
                );
            }
        }
        ...
        if (isset($opt['preauth']) && (integer)$opt['preauth'] > 0) {
            ...
            $headers['Expires']      = is_int($opt['preauth']) ? $opt['preauth']
                : strtotime($opt['preauth']);
        }
        ...
        $string_to_sign
            .=
            (($bucket === '' || $this->resource_prefix === '/' . $bucket) ? ''
                : ('/' . $bucket)) . $signable_resource . urldecode(
                $signable_query_string
            );

        // Hash the AWS secret key and generate a signature for the request.
        $signature = base64_encode(
            hash_hmac('sha1', $string_to_sign, $this->secret_key, true)
        );
        $request->add_header(
        ...
        if (isset($opt['preauth']) && (integer)$opt['preauth'] > 0) {
            return $this->request_url . $conjunction . 'AWSAccessKeyId='
            . $this->key . '&Expires=' . $headers['Expires'] . '&Signature='
        ...
        if ($this->use_batch_flow) {
            ...
            return $handle;
        } elseif (isset($opt['returnCurlHandle'])
            && $opt['returnCurlHandle'] === true
        ) {
        ...
        $data = new $this->response_class(
            $headers, $this->parse_callback($request->get_response_body()),
        ...
        ) // Temporary redirect to new endpoint.
        {
            $data = $this->authenticate(
                $bucket, $opt, $headers['location'], ++$redirects
            );
        } // Was it Amazon's fault the request failed? Retry the request until we reach $max_retries.
        elseif ((integer)$request->get_response_code() === 500
    ...
    public function validate_bucketname_create($bucket)
    {
        ...
        if (
            ...
            ||                  // Must not be null or false
            preg_match('/[^(a-z0-9\-\.)]/', $bucket)
            ||                 // Must be in the lowercase Roman alphabet, period or hyphen
            !preg_match('/^([a-z]|\d)/', $bucket)
            ||                    // Must start with a number or letter
            !(strlen($bucket) >= 3 && strlen($bucket) <= 63)
            ...
            ||                        // Bucket names cannot contain dashes next to periods
            preg_match('/(-|\.)$/', $bucket)
            ||                         // Bucket names should not end with a dash or period
            preg_match(
                '/^(?:[0-9]{1,3}\.){3}[0-9]{1,3}$/', $bucket
    ...
    public function validate_bucketname_support($bucket)
    {
        ...
        if (
            ($bucket === null || $bucket === false)
            ||                  // Must not be null or false
            preg_match('/[^(a-z0-9_\-\.)]/i', $bucket)
            ||               // Must be in the Roman alphabet, period, hyphen or underscore
            !preg_match('/^([a-z]|\d)/i', $bucket)
            ||                   // Must start with a number or letter
            !(strlen($bucket) >= 3 && strlen($bucket) <= 255)
    ...
    public function cache_callback(
        $bucket, $opt = null, $location = null, $redirects = 0
    ) {
        ...
        if (isset($response->body)
            && ($response->body instanceof SimpleXMLElement)
        ) {
    ...
    public function set_region($region)
    {
        ...
        switch ($region) {
            ...
            case self::REGION_EU_W1: // Ireland
                ...
                ); // Always use path-style access for EU endpoint.
                break;
    ...
    public function create_bucket(
        $bucket, $region, $acl = self::ACL_PRIVATE, $opt = null
    ) {
        // If the bucket contains uppercase letters...
        if (preg_match('/[A-Z]/', $bucket)) {
            // Throw a warning
            trigger_error(
                'Since DNS-valid bucket names cannot contain uppercase characters, "'
                . $bucket . '" has been automatically converted to "'
                . strtolower($bucket) . '"', E_USER_WARNING
            );
        ...
        if (!$this->validate_bucketname_create($bucket)) {
            ...
            throw new S3_Exception(
                '"' . $bucket
                . '" is not DNS-valid (i.e., <bucketname>.s3.amazonaws.com), and cannot be used as an S3 bucket name. Review "Bucket Restrictions and Limitations" in the S3 Developer Guide for more information.'
            );
            // @codeCoverageIgnoreEnd
        }
        ...
        switch ($region) {
            ...
            case self::REGION_EU_W1:    // Ireland
                $this->enable_path_style(
                ); // DNS-style doesn't seem to work for creation, only in EU. Switch over to path-style.
            ...
            default:
                $this->enable_path_style(false);
                $xml->LocationConstraint = str_replace(
                    array('s3-', '.amazonaws.com'), '', $region
                );
                $opt['body']             = $xml->asXML();
                break;
            // @codeCoverageIgnoreStart
    ...
    public function set_bucket_acl(
        $bucket, $acl = self::ACL_PRIVATE, $opt = null
    ) {
        // Add this to our request
        if (!$opt) {
            $opt = array();
        ...
        // @codeCoverageIgnoreStart
        if (!$this->credentials->canonical_id
            || !$this->credentials->canonical_name
        ) {
            // Fetch the data live.
        ...
        if (is_array($acl)) {
            $opt['body'] = $this->generate_access_policy(
                $this->credentials->canonical_id,
                $this->credentials->canonical_name, $acl
            );
        } else {
            $opt['body']                 = '';
    ...
    public function create_object($bucket, $filename, $opt = null)
    {
        ...
        if (isset($opt['meta'])) {
            foreach ($opt['meta'] as $meta_key => $meta_value) {
                ...
                $opt['headers']['x-amz-meta-' . strtolower(
    ...
    public function delete_objects($bucket, $opt = null)
    {
        ...
        if (!isset($opt['objects']) || !is_array($opt['objects'])) {
            throw new S3_Exception(
                'The ' . __FUNCTION__
                . ' method requires the "objects" option to be set as an array.'
            );
        }
    ...
    public function copy_object($source, $dest, $opt = null)
    {
        ...
        if (isset($source['bucket']) && isset($source['filename'])) {
            $opt['headers']['x-amz-copy-source']
                = '/' . $source['bucket'] . '/' . rawurlencode(
                    $source['filename']
                )
                . (isset($opt['versionId']) ? ('?' . 'versionId='
                    . rawurlencode($opt['versionId']))
            ...
            if (
                ...
                && !in_array(
                    ...
                    array_map('strtolower', array_keys($opt['headers']))
                )
            ) {
                $response = $this->get_object_headers(
                    $source['bucket'], $source['filename']
                );
                if ($response->isOK()) {
        ...
        if (isset($opt['ifNoneMatch'])) {
            $opt['headers']['x-amz-copy-source-if-none-match']
                = $opt['ifNoneMatch'];
            unset($opt['ifNoneMatch']);
        }
        if (isset($opt['ifUnmodifiedSince'])) {
            $opt['headers']['x-amz-copy-source-if-unmodified-since']
                = $opt['ifUnmodifiedSince'];
            unset($opt['ifUnmodifiedSince']);
        }
        if (isset($opt['ifModifiedSince'])) {
            $opt['headers']['x-amz-copy-source-if-modified-since']
                = $opt['ifModifiedSince'];
     ...
     * @link http://docs.amazonwebservices.com/AmazonS3/latest/dev/RESTAccessPolicy.html REST Access Control Policy
     */
    public function set_object_acl(
        $bucket, $filename, $acl = self::ACL_PRIVATE, $opt = null
    ) {
        // Add this to our request
        if (!$opt) {
            $opt = array();
    ...
    public function generate_access_policy(
        $canonical_id, $canonical_name, $users
    ) {
    ...
    public function enable_logging(
        $bucket, $target_bucket, $target_prefix, $opt = null
    ) {
        // Add this to our request
        if (!$opt) {
            $opt = array();
    ...
    public function if_bucket_exists($bucket)
    {
        if ($this->use_batch_flow) {
            throw new S3_Exception(
                __FUNCTION__ . '() cannot be batch requested'
            );
        }
     ...
     * @return integer|string The number of bytes as an integer, or the friendly format as a string.
     */
    public function get_object_filesize(
        $bucket, $filename, $friendly_format = false
    ) {
        if ($this->use_batch_flow) {
            throw new S3_Exception(
                __FUNCTION__ . '() cannot be batch requested'
    ...
    public function change_content_type(
        $bucket, $filename, $contentType, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function change_storage_redundancy(
        $bucket, $filename, $storage, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function delete_all_object_versions($bucket, $pcre = null)
    {
        ...
        if ($versions->body->DeleteMarker() && $versions->body->Version()) {
            $markers = array_merge(
                $versions->body->DeleteMarker()->getArrayCopy(),
                $versions->body->Version()->getArrayCopy()
            );
    ...
    public function get_object_metadata($bucket, $filename, $opt = null)
    {
        ...
        $this->batch($batch)->get_object_headers(
            $bucket, $filename
        ); // Get content-type
        ...
        $contents = json_decode(
            json_encode(
                $response[2]->body->query('descendant-or-self::Contents')
                    ->first()
            ), true
        );
        $data     = array_merge(
            $data, (is_array($contents) ? $contents : array())
        );
        ...
        foreach (
            array('_info', 'x-amz-id-2', 'x-amz-request-id', 'cneonction',
                  'server', 'content-length', 'content-type', 'etag') as $header
        ) {
        ...
        if (count($grants) > 0) {
            foreach ($grants as $grant) {
                $dgrant = array(
                    'id'         => (string)$this->util->try_these(
                        array('ID', 'URI'), $grant->Grantee
                    ),
    ...
    public function get_object_url(
        $bucket, $filename, $preauth = 0, $opt = null
    ) {
        // Add this to our request
        if (!$opt) {
            $opt = array();
    ...
    public function enable_versioning($bucket, $opt = null)
    {
        ...
        if (isset($opt['MFASerial']) && isset($opt['MFAToken'])
            ...
            $opt['headers']['x-amz-mfa'] = ($opt['MFASerial'] . ' '
                . $opt['MFAToken']);
        }
        // @codeCoverageIgnoreEnd
    ...
    public function list_bucket_object_versions($bucket, $opt = null)
    {
        ...
        foreach (
            array('delimiter', 'key-marker', 'max-keys', 'prefix',
                  'version-id-marker') as $param
        ) {
            if (isset($opt[$param])) {
                $opt['query_string'][$param] = $opt[$param];
                unset($opt[$param]);
     ...
     */
    public function create_bucket_notification(
        $bucket, $topic_arn, $event, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function upload_part($bucket, $filename, $upload_id, $opt = null)
    {
        ...
        if (!isset($opt['fileUpload']) || !isset($opt['partNumber'])) {
            throw new S3_Exception(
                'The `fileUpload` and `partNumber` options are both required in '
                . __FUNCTION__ . '().'
            );
        }
    ...
    public function abort_multipart_upload(
        $bucket, $filename, $upload_id, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function complete_multipart_upload(
        $bucket, $filename, $upload_id, $parts, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
    ...
    public function list_multipart_uploads($bucket, $opt = null)
    {
        ...
        foreach (
            array('key-marker', 'max-uploads', 'upload-id-marker') as $param
        ) {
            if (isset($opt[$param])) {
                $opt['query_string'][$param] = $opt[$param];
                unset($opt[$param]);
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function copy_part(
        $source, $dest, $upload_id, $part_number, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        // Partial content range
        if (isset($opt['range'])) {
            $opt['headers']['x-amz-copy-source-range']
                = 'bytes=' . $opt['range'];
        }
    ...
    public function create_mpu_object($bucket, $filename, $opt = null)
    {
        ...
        if (!isset($opt['fileUpload'])) {
            throw new S3_Exception(
                'The `fileUpload` option is required in ' . __FUNCTION__ . '().'
            );
        } elseif (is_resource($opt['fileUpload'])) {
            $opt['limit']    = 1; // We can only read from this one resource.
            $upload_position = isset($opt['seekTo']) ? (integer)$opt['seekTo']
                : ftell($opt['fileUpload']);
            $upload_filesize = isset($opt['headers']['Content-Length'])
                ? (integer)$opt['headers']['Content-Length'] : null;
        ...
        if ($upload_position === false || !isset($upload_filesize)
            || $upload_filesize === false
            || $upload_filesize < 0
        ) {
            throw new S3_Exception(
                'The size of `fileUpload` cannot be determined in '
                . __FUNCTION__ . '().'
            );
        }
        ...
        if (isset($opt['uploadId'])) {
            ...
            $_opt = array();
            foreach (
                array('contentType', 'acl', 'storage', 'headers', 'meta') as
                $param
            ) {
                if (isset($opt[$param])) {
            ...
            $upload = $this->initiate_multipart_upload(
                $bucket, $filename, $_opt
            );
            if (!$upload->isOK()) {
                return $upload;
            }
        ...
        $pieces = $this->get_multipart_counts(
            $upload_filesize, (integer)$opt['partSize']
        );

        // Queue batch requests
        $batch = new CFBatchRequest(
            isset($opt['limit']) ? (integer)$opt['limit'] : null
        );
        ...
        foreach ($batch_responses as $i => $response) {
            $parts[] = array('PartNumber' => ($i + 1),
                             'ETag'       => $response->header['etag']);
        }

        return $this->complete_multipart_upload(
            $bucket, $filename, $upload_id, $parts
        );
    }
