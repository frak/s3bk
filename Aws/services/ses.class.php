...
class AmazonSES extends CFRuntime
{
    ...
    public function disable_ssl()
    {
        throw new Email_Exception(
            'SSL/HTTPS is REQUIRED for Amazon Email Service and cannot be disabled.'
        );
    }
    ...
    public function send_email($source, $destination, $message, $opt = null)
    {
        ...
        if (isset($opt['ReplyToAddresses'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'ReplyToAddresses' => (is_array($opt['ReplyToAddresses'])
    ...
    public function send_raw_email($raw_message, $opt = null)
    {
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'RawMessage' => (is_array($raw_message) ? $raw_message
                    : array($raw_message))
            ), 'member'
        )
        );
        ...
        if (isset($opt['Destinations'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'Destinations' => (is_array($opt['Destinations'])
                        ? $opt['Destinations'] : array($opt['Destinations']))
                ), 'member'
            )
            );
