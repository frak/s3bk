...
class AmazonSQS extends CFRuntime
{
    ...
    public function add_permission(
        $queue_url, $label, $aws_account_id, $action_name, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            ...
            array(
                'AWSAccountId' => (is_array($aws_account_id) ? $aws_account_id
                    : array($aws_account_id))
            )
        )
        );
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'ActionName' => (is_array($action_name) ? $action_name
                    : array($action_name))
            )
        )
        );
    ...
    public function change_message_visibility(
        $queue_url, $receipt_handle, $visibility_timeout, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function change_message_visibility_batch(
        $queue_url, $change_message_visibility_batch_request_entry, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'ChangeMessageVisibilityBatchRequestEntry' => (is_array(
                    $change_message_visibility_batch_request_entry
                ) ? $change_message_visibility_batch_request_entry
                    : array($change_message_visibility_batch_request_entry))
            )
        )
        );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function delete_message_batch(
        $queue_url, $delete_message_batch_request_entry, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'DeleteMessageBatchRequestEntry' => (is_array(
                    $delete_message_batch_request_entry
                ) ? $delete_message_batch_request_entry
                    : array($delete_message_batch_request_entry))
            )
        )
        );
    ...
    public function get_queue_attributes($queue_url, $opt = null)
    {
        ...
        if (isset($opt['AttributeName'])) {
            $opt = array_merge(
                $opt, CFComplexType::map(
                array(
                    'AttributeName' => (is_array($opt['AttributeName'])
                        ? $opt['AttributeName'] : array($opt['AttributeName']))
                )
            )
            );
     ...
     * @return CFResponse A <CFResponse> object containing a parsed HTTP response.
     */
    public function send_message_batch(
        $queue_url, $send_message_batch_request_entry, $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        $opt = array_merge(
            $opt, CFComplexType::map(
            array(
                'SendMessageBatchRequestEntry' => (is_array(
                    $send_message_batch_request_entry
                ) ? $send_message_batch_request_entry
                    : array($send_message_batch_request_entry))
            )
        )
        );
