...
class CFCredentials
{
    ...
    public static function set(array $credential_sets)
    {
        ...
        if (count($credential_sets) === 1) {
            ...
            throw new CFCredentials_Exception(
                'If more than one credential set is provided, a default credential set (identified by the key "'
                . self::DEFAULT_KEY . '") must be specified.'
            );
        }
        ...
        foreach ($credential_sets as $credential_name => &$credential_set) {
            if (is_array($credential_set)) {
                foreach (
                    $credential_set as $credential_key => &$credential_value
                ) {
                    if ($credential_key === self::INHERIT_KEY) {
                        if (!isset($credential_sets[$credential_value])) {
                            throw new CFCredentials_Exception(
                                'The credential set, "' . $credential_value
                                . '", does not exist and cannot be inherited.'
                            );
                        }

                        $credential_set = array_merge(
                            $credential_sets[$credential_value], $credential_set
                        );
        ...
        if (is_string($default)) {
            if (!isset($credential_sets[$default])) {
                throw new CFCredentials_Exception(
                    'The credential set, "' . $default
                    . '", does not exist and cannot be used as the default credential set.'
                );
            }
    ...
    public static function get($credential_name = self::DEFAULT_KEY)
    {
        ...
        if (!isset(self::$credentials[$credential_name])) {
            throw new CFCredentials_Exception(
                'The credential set, "' . $credential_name
