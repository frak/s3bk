...
class RequestCore
{
    ...
    public function streaming_read_callback($curl_handle, $file_handle, $length)
    {
        ...
        if ($this->registered_streaming_read_callback) {
            call_user_func(
                $this->registered_streaming_read_callback, $curl_handle,
                $file_handle, $out
            );
        }
    ...
    public function streaming_write_callback($curl_handle, $data)
    {
        ...
        while ($written_total < $length) {
            $written_last = fwrite(
                $this->write_stream, substr($data, $written_total)
            );
        ...
        if ($this->registered_streaming_write_callback) {
            call_user_func(
                $this->registered_streaming_write_callback, $curl_handle,
                $written_total
            );
        }
    ...
    public function prep_request()
    {
        ...
        curl_setopt(
            $curl_handle, CURLOPT_CLOSEPOLICY,
        ...
        if ($this->cacert_location === true) {
            curl_setopt(
                $curl_handle, CURLOPT_CAINFO, dirname(__FILE__) . '/cacert.pem'
            );
        ...
        if ($this->proxy) {
            ...
            if (isset($this->proxy['user']) && isset($this->proxy['pass'])) {
                curl_setopt(
                    $curl_handle, CURLOPT_PROXYUSERPWD,
        ...
        switch ($this->method) {
            case self::HTTP_PUT:
                ...
                if (isset($this->read_stream)) {
                    ...
                    curl_setopt(
                        $curl_handle, CURLOPT_INFILESIZE,
                        $this->read_stream_size
                    );
            ...
            case self::HTTP_HEAD:
                curl_setopt(
                    $curl_handle, CURLOPT_CUSTOMREQUEST, self::HTTP_HEAD
                );
    ...
    public function process_response($curl_handle = null, $response = null)
    {
        ...
        if (is_resource($this->curl_handle)) {
            ...
            $header_size            = curl_getinfo(
                $this->curl_handle, CURLINFO_HEADER_SIZE
            );
            $this->response_headers = substr($this->response, 0, $header_size);
            $this->response_body    = substr($this->response, $header_size);
            $this->response_code    = curl_getinfo(
                $this->curl_handle, CURLINFO_HTTP_CODE
            );
    ...
    public function send_request($parse = false)
    {
        ...
        $parsed_response = $this->process_response(
            $curl_handle, $this->response
        );
    ...
    public function send_multi_request($handles, $opt = null)
    {
        ...
        do {
            ...
            while (($status = curl_multi_exec($multi_handle, $active))
            ...
            while ($done = curl_multi_info_read($multi_handle)) {
                ...
                if ($done['result'] > 0) {
                    throw new RequestCore_Exception(
                        'cURL resource: ' . (string)$done['handle']
                        . '; cURL error: ' . curl_error($done['handle']) . ' ('
                        . $done['result'] . ')'
                    );
            ...
            foreach ($to_process as $pkey => $done) {
                $response           = $http->process_response(
                    $done['handle'], curl_multi_getcontent($done['handle'])
                );
                $key                = array_search(
                    $done['handle'], $handle_list, true
                );
