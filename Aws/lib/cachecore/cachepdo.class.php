...
class CachePDO extends CacheCore implements ICacheCore
{
    ...
    public function __construct($name, $location, $expires, $gzip = true)
    {
        ...
        if (stripos($location, '://') === false) {
            ...
            $this->dsn = parse_url($location);
            $this->dsn_string
                       = $this->dsn['scheme'] . ':host=' . $this->dsn['host']
                . ((isset($this->dsn['port'])) ? ';port=' . $this->dsn['port']
                    : '') . ';dbname=' . substr($this->dsn['path'], 1);
        }
        ...
        $this->create = $this->pdo->prepare(
            "INSERT INTO cache (id, expires, data) VALUES (:id, :expires, :data)"
        );
        $this->read   = $this->pdo->prepare(
            "SELECT id, expires, data FROM cache WHERE id = :id"
        );
        $this->reset  = $this->pdo->prepare(
