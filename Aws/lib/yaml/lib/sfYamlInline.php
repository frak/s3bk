...
class sfYamlInline
{
    ...
    static public function dump($value)
    {
        ...
        switch (true) {
            ...
            case is_numeric($value):
                return is_infinite($value) ? str_ireplace(
                    'INF', '.Inf', strval($value)
    ...
    static protected function dumpArray($value)
    {
        ...
        if (
            ...
            || (count($keys) > 1
                && array_reduce(
                    $keys,
                    create_function('$v,$w', 'return (integer) $v + $w;'), 0
     ...
     */
    static public function parseScalar(
        $scalar, $delimiters = null, $stringDelimiters = array('"', "'"),
        &$i = 0, $evaluate = true
    ) {
    ...
    {
        if (!preg_match(
            '/' . self::REGEX_QUOTED_STRING . '/A', substr($scalar, $i), $match
        )
        ) {
            throw new InvalidArgumentException(
                sprintf(
                    'Malformed inline YAML string (%s).', substr($scalar, $i)
                )
            );
        }
