...
class sfYamlParser
{
                        ...
                        if (isset($values['value'])
                            ...
                            if (!is_array($parsed)) {
                                ...
                                if (isset($parsed[0])) {
                                    ...
                                    foreach (
                                        array_reverse($parsed) as $parsedItem
                                    ) {
    ...
    {
        return strlen($this->currentLine) - strlen(
    ...
    protected function getNextEmbedBlock($indentation = null)
    {
        ...
        if (null === $indentation) {
            ...
            if (!$this->isCurrentLineEmpty() && 0 == $newIndent) {
                throw new InvalidArgumentException(
                    sprintf(
                        'Indentation problem at line %d (%s)',
                        $this->getRealCurrentLineNb() + 1, $this->currentLine
                    )
                );
            }
    ...
    protected function parseValue($value)
    {
        if ('*' === substr($value, 0, 1)) {
            ...
            if (!array_key_exists($value, $this->refs)) {
                throw new InvalidArgumentException(
                    sprintf(
                        'Reference "%s" does not exist (%s).', $value,
                        $this->currentLine
                    )
                );
            }
        ...
        if (preg_match(
            ...
            return $this->parseFoldedScalar(
                $matches['separator'], preg_replace('#\d+#', '', $modifiers),
     ...
     */
    protected function parseFoldedScalar(
