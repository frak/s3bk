...
class S3BrowserUpload extends AmazonS3
{
     ...
     */
    public function generate_upload_parameters(
        $bucket, $expires = '+1 hour', $opt = null
    ) {
        if (!$opt) {
            $opt = array();
        }
        ...
        if ($expires) {
            ...
            $expires              = $this->util->convert_date_to_iso8601(
                $expires
            );
        ...
        if (isset($opt['success_action_status'])
            && !empty($opt['success_action_status'])
        ) {
            $form['inputs']['success_action_status']
                                    = (string)$opt['success_action_status'];
        ...
        foreach ($opt as $param_key => $param_value) {
            if ($param_value[0] === '^') {
                ...
                $param_value                = preg_replace(
                    '/\$\{(\w*)\}/', '', (string)$param_value
                );
                $policy['conditions'][]     = array('starts-with',
                                                    '$' . $param_key, (substr(
                        (string)$param_value, 1
                    ) ? substr((string)$param_value, 1) : ''));
            } else {
        ...
        // Add signature
        $form['inputs']['signature'] = base64_encode(
            hash_hmac('sha1', $json_policy_b64, $this->secret_key, true)
        );
    ...
    public static function domain()
    {
        if (isset($_SERVER['SERVER_NAME']) && isset($_SERVER['SERVER_PORT'])) {
            return $_SERVER['SERVER_NAME'] . ((integer)$_SERVER['SERVER_PORT']
