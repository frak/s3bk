...
class S3StreamWrapper extends AmazonS3
{
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $pattern
            = '/^' . self::regex_token(substr($path_name, 1)) . '(.*)[^\/$]/';

        $this->file_list = $this->get_object_list(
            $bucket_name, array(
            'pcre' => $pattern
        )
        );
    ...
    public function stream_read($count)
    {
        ...
        if ($this->seek_position !== 0) {
            $response = $this->get_object(
                $bucket_name, substr($path_name, 1), array(
        ...
        $this->seek_position = isset($response->header['content-length'])
    ...
    public function stream_write($data)
    {
        ...
        if ($this->seek_position !== 0) {
            trigger_error(
                __CLASS__
                . ' is unable to append to a file, so the seek position for this resource must be rewound to position 0.'
            );
        } else {
        ...
        $this->seek_position
                   = $response->header['x-aws-requestheaders']['Content-Length'];
        $this->eof = true;
    ...
    public function url_stat($path, $flags)
    {
        ...
        */
        ...
        if (!$path_name) // Root of the bucket
        {
            ...
            $mode = (str_replace('//', '/', substr($path_name, 1) . '/')
                === (string)$file->Key)
                ? octdec('0040777')
                : octdec(
                    '0100777'
                ); // Directory, Owner R/W : Regular File, Owner R/W
        } else {
            $mode = octdec('0100777');
        }
        ...
        $out[8]  = $out['atime'] = (isset($file) ? date(
            'U', strtotime((string)$file->LastModified)
        ) : 0);
        $out[9]  = $out['mtime'] = (isset($file) ? date(
            'U', strtotime((string)$file->LastModified)
        ) : 0);
        $out[10] = $out['ctime'] = (isset($file) ? date(
            'U', strtotime((string)$file->LastModified)
        ) : 0);
